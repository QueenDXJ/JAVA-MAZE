...
public class MazeViewer {
	...
	*/
	private static MazeFrame readMazeFile(String fileName) throws IOException {
		...
		//file should be put in the pa3 dir
		Scanner in = new Scanner(file);
		int numRows = in.nextInt();
		int numCols = in.nextInt();
		in.nextLine();
		boolean[][] maze = setMaze(in, numRows, numCols);
		MazeCoord startLoc = new MazeCoord(in.nextInt(),in.nextInt());
		MazeCoord exitLoc = new MazeCoord(in.nextInt(),in.nextInt());
		in.close();
		return new MazeFrame(maze,startLoc,exitLoc);
	}
	/**
	 set the maze according to the scanner's information
	
	@param Scanner, the number of rows and the number of columns
	@returns a boolean[][] contained the information of maze.
	     
	*/
	private static boolean[][] setMaze(Scanner in, int numRows, int numCols){
		boolean[][] maze = new boolean[numRows][numCols];
		String[] reader = new String[numRows];
		char[] eachLine = new char[numCols];
		for(int row=0; row<numRows; row++){
			reader[row] = in.nextLine();
			eachLine = reader[row].toCharArray();
			for(int col=0; col<numCols; col++){
				maze[row][col] = (eachLine[col] == WALL_CHAR)? true:false; 
			}
		}
		return maze;
